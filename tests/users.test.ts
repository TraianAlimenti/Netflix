...
describe("Resources test", () => {
  const RESOURCE_PROPERTY_NAMES = ['id','name','email'];
  ...
  it("get single users", async () => {
    const response = await fetch(TARGET_URL + "/users/1");
    const data = await response.json();
  ...
  it("create users", async () => {
    const response = await fetch(TARGET_URL + "/users/", {
      ...
      body: JSON.stringify({"name": "Marta","email": "Marta@mail.com"}),
    });
  ...
  it("patch users", async () => {
    const response = await fetch(TARGET_URL + "/users/3", {
      method: "PATCH",
      headers: { "Content-type": "application/json; charset=UTF-8" },
      body: JSON.stringify({"email": "pepe@gmail.com"}),
    });
    const data = await response.json();
    expect(response.status).toBe(200);
    expect(Object.keys(data)).toStrictEqual(RESOURCE_PROPERTY_NAMES); // check that we only have the required fields.
